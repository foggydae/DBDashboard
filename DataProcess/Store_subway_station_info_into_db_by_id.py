...
# prepare the data from the csv file into station(id as key) structure
def generate_station_dict (csvDf):
	...
	for index, row in csvDf.iterrows():
		...
		if math.isnan(row['LONGITUDE']):
			...
			print("ERROR:", row['STATION_NUM'], "has no longitude and latitude data.")
			continue

		if row['STATION_NUM'] not in stationDict:
			stationDict[row['STATION_NUM']] = {
			...
			if not math.isnan(row['ADJ_STATION']):
				stationDict[row['STATION_NUM']]['ADJ_STATION'].append(format_station_ID(int(row['ADJ_STATION'])))
			if not math.isnan(row['ADJ_STATION_2']):
				stationDict[row['STATION_NUM']]['ADJ_STATION'].append(format_station_ID(int(row['ADJ_STATION_2'])))
		else:
			print("???")
...
if __name__ == '__main__':
	...
	db['station_info_by_id'].insert_many(station_list)
